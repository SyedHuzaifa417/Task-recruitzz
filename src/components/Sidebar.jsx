        ...
        {items.map((item) => (
          ...
          >
            <button
              ...
              className={clsx(
                "w-full relative flex flex-col items-center py-4 text-sm font-medium rounded-l-2xl",
                currentPath === item.id
                  ? "text-[#6968EC]"
            ...
            >
              ...
              {item.count > 0 && (
                <span className="absolute right-5 top-2 text-red-100 bg-[#F88080] px-1 rounded-full text-xs">
                  {item.count}
                </span>
              )}
            </button>
